...
Plot 1 and plot 3 have the same color scales as they are both coloring True/False values, however plot 2 has a different color scale as it is now coloring a numeric variable and thus coloring based on 0 or 1, which is why there are only two colors shown instead of a gradient per the key. This is becasue the numeric variable was created by coercing the logical vector, thus turning the Yes and No into 0 and 1. Plot 3 has the same color scales as plot 1, since the factor vector was created by coercing the logical vector and thus for bot variables the values are still in True/False. 